...
@WebServlet("/DeletePaymentAPI")
public class DeletePaymentAPI extends HttpServlet {
    ...
    protected void doDelete(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            try (Connection conn = DriverManager.getConnection(URL, USER, PASS)) {
                // Check existence
                String selectSql = "SELECT * FROM payment WHERE Membership_ID = ?";
                try (PreparedStatement selectStmt = conn.prepareStatement(selectSql)) {
                    ...
                    try (ResultSet rs = selectStmt.executeQuery()) {
                        if (!rs.next()) {
                            ...
                            JSONObject obj = new JSONObject();
                            obj.put("success", false);
                            obj.put("error", "Record not found");
                            response.getWriter().print(obj.toJSONString());
                            return;
                        }
                    }
                }
                String deleteSql = "DELETE FROM payment WHERE Membership_ID = ?";
                try (PreparedStatement deleteStmt = conn.prepareStatement(deleteSql)) {
                    ...
                    if (affected > 0) {
                        ...
                        obj.put("message", "Payment record deleted successfully");
                    } else {
                        response.setStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
                        obj.put("success", false);
                        obj.put("error", "Failed to delete payment record");
                    }
                    response.getWriter().print(obj.toJSONString());
                }
            }
